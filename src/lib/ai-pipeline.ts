...
// A sync pass can hand this 30-40+ unmatched emails at once (routine now
// that internal/automated mail is classified too). The batch used to share
// one fixed 1200-token ceiling no matter how many emails were in it --
// harmless for 2-3 emails, but for anything past ~4 it guaranteed the
// model's JSON got cut off mid-array, every reply in the WHOLE batch fell
// back to the generic "we'll review manually" placeholder, and nothing
// surfaced this beyond a console.error nobody was watching -- 881 emails in
// a single week got that placeholder instead of a real answer. Chunking
...
/**
 * Same as generateReplyWithAI but for N unmatched emails, chunked into
 * groups of BATCH_CHUNK_SIZE so each request's token budget actually fits
 * its own size -- used by the live IMAP sync loop when more than one
 * unseen message needs a fresh reply. Chunks run sequentially (gentler on
 * rate limits than firing them all at once) and a failure in one chunk
