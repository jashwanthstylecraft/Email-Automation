 ...
 *
 * `htmlBody` is an optional pre-rendered HTML version (used when a template
 * has an inline image -- see renderImagePlaceholders in ai-pipeline.ts).
 * When omitted, the HTML is generated from the plain-text `body` exactly as
 * before. `body` itself should always be a plain-text-safe fallback (no raw
 * HTML), since it's also what non-HTML mail clients see.
 */
export async function sendOutgoingMail(
  ...
  if (!smtpUser || !smtpPassword || !smtpHost) {
    ...
    if (htmlBody) console.log(`[Has inline image(s) -- HTML body ${htmlBody.length} chars]`);
    console.log('------------------------------------------------------------');
    return false;
  }
  ...
  try {
    ...
    const info = await transporter.sendMail({
      ...
      text: body,
      html: htmlBody ?? `<div style="font-family: sans-serif; white-space: pre-wrap; line-height: 1.6; color: #1f2937;">${body}</div>`,
    });
